...
namespace RouterDetector.CaptureConsole.DetectionProtocols
{
    public class DdosDetector : IDetector
    {
        ...
        public DetectionResult? Analyze(NetworkPacket packet)
        {
            ...
            {
                return new DetectionResult(true, packet,
                    $"Possible DDoS attack on {packet.DestinationIp} ({_targetHits[packet.DestinationIp].Count} packets in {_windowSeconds}s)",
                    ThreatSeverity.Critical, ProtocolName);
            }
            return null;
        }
    }
